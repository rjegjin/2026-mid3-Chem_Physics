...
# 현재 상태
- [x] 단원별 요약본 8종(학생용·교사용) 신설 + 인쇄 PDF 생성 스크립트, PDF를 저장소에 포함
- [x] 581px 과밀 0장(46페이지 전수), 사라져 있던 슬라이드 2장 복구(섹션 중첩·slide-content 이탈), 구조 검사 2종 추가, 인라인 수식 스크롤 남용 제거, 모바일 감사 430px 정상화, 19강 은하수 사진
- [x] 죽은 polyfill.io 제거(5개 페이지 40초 행), MathJax 오버플로 근본 수정(mjx-container 격리 + 감사 정정), 슬라이드 번호 중복 복구, 로드맵 미결 2건 종결
- [x] 무기화학 과정을 adv_inorganic/ 폴더로 분리 (플랫폼 css/js는 공유 유지), 검사 하네스 파일명 의존·비재귀 glob·render_check 이름 충돌 수정
