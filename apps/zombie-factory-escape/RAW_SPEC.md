# 프로젝트 명세서: 좀비 공장 화학 반응식 방탈출 웹앱

## 1. 프로젝트 개요

- 이름: 좀비 공장 (화학 반응식 퀘스트)
- 목적: 중학교 3학년 과학 `화학 반응의 규칙` 단원을 복습하기 위한 반응형 교육용 방탈출 게임
- 기술 스택: React 19, Vite 8, Tailwind CSS 3, Lucide React
- 현재 구현 상태:
  - 10분 제한 시간 타이머
  - 인벤토리 기반 단서 수집
  - 오답 노트 누적
...
```text
/home/rjegj/projects/2026-mid3-Chem_Physics/apps/zombie-factory-escape
```

이 앱은 기존 수업 저장소 `2026-mid3-Chem_Physics` 안의 서브프로젝트로 배치되어 있습니다.

## 3. 파일 구조

```text
zombie-factory-escape/
├── index.html
├── package.json
├── package-lock.json
├── postcss.config.js
...
기본 동작은 결과를 화면에 JSON으로 출력하는 것입니다.

선택적으로 아래 전역 변수를 설정하면 API로도 전송합니다.
...
```json
{
  ...
  "playSummary": {
    ...
    "finalPassword": "427",
    "submissionStatus": "json-only"
  },
  "wrongAnswerNotes": [
    {
      "step": 1,
      "title": "어둠 속의 불빛",
      "note": "양초 변화 문제에서 물리 변화 선택"
    }
  ]
}
```
...
<html lang="ko">
  <head>
    ...
    <meta
      name="description"
      content="중학교 3학년 화학 반응의 규칙 단원을 위한 반응형 교육용 방탈출 웹앱"
    />
    <title>좀비 공장 | 화학 반응식 퀘스트</title>
  </head>
  <body>
    <div id="root"></div>
...
:root {
  font-family: 'Pretendard Variable', 'SUIT Variable', system-ui, sans-serif;
  color: #f7f7ef;
  background:
    radial-gradient(circle at top, rgba(27, 94, 72, 0.32), transparent 40%),
    linear-gradient(180deg, #07130f 0%, #091a15 45%, #030806 100%);
  font-synthesis: none;
...
## 7. 다음 AI에게 바로 줄 작업 예시

```text
위 명세의 React/Vite 프로젝트는 이미 구현되어 있습니다.
다음 작업을 이어서 해 주세요.
1. 결과 제출용 실제 백엔드 API 스펙에 맞게 fetch 요청을 수정
2. 사운드 파일(mp3/wav) 기반 효과음으로 교체하거나 토글 기능 추가
3. 교사용 결과 대시보드 또는 CSV 내보내기 기능 추가
4. 퀘스트 문제를 학급 수준에 맞게 쉽게/어렵게 조정
```
